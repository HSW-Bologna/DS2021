...
#define DESC parameters_desc
    ...
    switch (s->type) {
        case DRYER_PROGRAM_STEP_TYPE_DRYING:
            ...
            pars[i++] = PARAMETER(&s->drying.duration,              0,     60*60,    60,    ((pudata_t){.t = PTYPE_TIME, .desc = DESC[PARAMETERS_DESC_DURATA], .fmt=msfmt, .min_sec = 1}),                     USER_BITS);
            ...
            pars[i++] = PARAMETER(&s->drying.speed,            0,     100,       40,    ((pudata_t){.t = PTYPE_NUMBER, .desc = DESC[PARAMETERS_DESC_VELOCITA], .fmt="%i rpm"}),               USER_BITS);
            pars[i++] = PARAMETER(&s->drying.temperature,            0,     100,       40,    ((pudata_t){.t = PTYPE_NUMBER, .desc = DESC[PARAMETERS_DESC_TEMPERATURA], .fmt="%i C"}),               USER_BITS);
            pars[i++] = PARAMETER(&s->drying.humidity,            0,     100,       40,    ((pudata_t){.t = PTYPE_NUMBER, .desc = DESC[PARAMETERS_DESC_UMIDITA], .fmt="%i%%"}),               USER_BITS);
            // clang-format on
            break;
        ...
        case DRYER_PROGRAM_STEP_TYPE_COOLING:
            ...
            pars[i++] = PARAMETER(&s->cooling.duration,              0,     60*60,    60,    ((pudata_t){.t = PTYPE_TIME, .desc = DESC[PARAMETERS_DESC_DURATA], .fmt=msfmt, .min_sec = 1}),                     USER_BITS);
            ...
            pars[i++] = PARAMETER(&s->cooling.temperature,            0,     100,       40,    ((pudata_t){.t = PTYPE_NUMBER, .desc = DESC[PARAMETERS_DESC_TEMPERATURA], .fmt="%i C"}),               USER_BITS);
        ...
        case DRYER_PROGRAM_STEP_TYPE_UNFOLDING:
            // clang-format off
            pars[i++] = PARAMETER(&s->unfolding.max_duration,             60,     600,     120,    ((pudata_t){.t = PTYPE_TIME, .desc = DESC[PARAMETERS_DESC_DURATA_MASSIMA], .fmt = msfmt, .min_sec = 1}),                     USER_BITS);
            ...
            pars[i++] = PARAMETER(&s->unfolding.speed,            0,     100,       40,    ((pudata_t){.t = PTYPE_NUMBER, .desc = DESC[PARAMETERS_DESC_VELOCITA], .fmt="%i rpm"}),               USER_BITS);
